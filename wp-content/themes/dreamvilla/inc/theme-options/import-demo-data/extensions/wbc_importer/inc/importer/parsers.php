		...
		if ( isset($result) && defined('IMPORT_DEBUG') && IMPORT_DEBUG ) {
			...
			if ( 'SimpleXML_parse_error' == $result->get_error_code() ) {
				foreach  ( $result->get_error_data() as $error )
					printf( esc_html__('%s : %s %s','dreamvilla-multiple-property'),$error->line,$error->column,esc_html($error->message) ). "\n";
			} else if ( 'XML_parse_error' == $result->get_error_code() ) {
				$error = $result->get_error_data();
				printf( esc_html__('%s : %s %s','dreamvilla-multiple-property'),$error[0],$error[1],esc_html($error[2]) );
			}
			echo '</pre>';
...
class WXR_Parser_XML {
	...
	function parse( $file ) {
		...
		if ( ! xml_parse( $xml, $file_data, true ) ) {
...
class WXR_Parser_Regex {
	...
	function get_contents( $filename, $mode = 'r' ) {
		if ( $this->has_gzip )
			return gzopen( $filename, $mode );
		return get_contents( $filename, $mode );
	}
